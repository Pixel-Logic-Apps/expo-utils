...
The plugin also enables **`analyticsAdSupport` by default** (opt-out): at **prebuild** (`withDangerousMod` → `withAnalyticsAdSupport`) it injects `$RNFirebaseAnalyticsEnableAdSupport = true` into the iOS `Podfile` (anchored after `require 'json'`, idempotent via tag + skip-if-present + try/catch). `RNFBAnalytics.podspec` reads that global at pod install and links `AdSupport.framework`, giving Firebase Analytics IDFA access → audience demographics/interests on iOS (parity with Android). RNFirebase's analytics config plugin has **no** option for this (only `withoutAdIdSupport` / `googleAppMeasurementOnDeviceConversion`), hence the Podfile injection. **Privacy:** this collects IDFA in production — requires ATT (expo-utils already prompts) + IDFA declared in the privacy manifest / App Store. It's default-on because expo-utils is ads-first (apps already collect IDFA via AdMob); turn it **off** for a no-ads app with `["expo-utils", { "analyticsAdSupport": false }]`. Needs `prebuild --clean` + rebuild to take effect.
